...
description: Install a verified Octoform 0.5 patch and confirm the executable before connecting to GitHub.
---

# Install Octoform

Choose the `MAJOR.MINOR` documentation line that matches the package, then pin
an exact patch. This page currently verifies `0.5.0`; the
...
Octoform `0.5` requires Node.js 20 or newer. Use a supported Node.js release
...
!!! note "Documentation and package versions"
    ...
    patch. The selector remains `0.5` for every compatible `0.5.x` package;
