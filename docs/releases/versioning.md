...
```text
Octoform package line:  0.3.x
Validated package:      0.3.2
Version selector:       0.3
Canonical path:         /octoform-docs/0.3/
```

A patch-only release can update the changelog, exact installation commands,
evidence, and availability notices while the selector remains `0.3`. A change
...
General pages describe the line-wide `0.3` contract. When behavior was added
or corrected after `0.3.0`, the relevant page states **Available since
...
Visible site version:   0.3
```

Consequently, documentation publication `v0.3.25` can update the `0.3` site
while its build remains validated with Octoform `0.3.2`. The GitHub Release
...
| --- | --- | --- |
| `/octoform-docs/0.3/` | Current verified documentation for compatible Octoform `0.3.x` packages | Updated by approved publications in the line |
| `/octoform-docs/0.3.x/` | Backward-compatible redirect from an exact patch URL to `/0.3/`; not shown in the selector | Moving redirect |
